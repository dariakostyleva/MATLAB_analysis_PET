...
% pet_shift = 2900;
% z12=load('D:\PET_data_feb_2021\C10_001.mat');
% ifc_hist = load("hist_0219_C10_001_200msec_bins.txt");
% ifc_shifted = ifc_hist(:,1)*1000.+3500;
% pet_shift = 56400;
% z12=load('D:\PET_data_feb_2021\C11_007.mat');
...
tt = events(nevents-1,9); % last time stamp ie total time of file in ms
clear z12; 
...
[h_c,h_e] = histcounts(events(:,9),binning);
h_e(length(h_e)) = [];
...
for i=1:nevents
    ...
    if events(i,9) < pet_shift
        continue;
    end
    
    if(events(i,9) >= ifc_shifted(length(ifc_shifted)))
        break;
    end

    for k=2:length(ifc_hist)
       %k
       if and(events(i,9) >= ifc_shifted(k-1),and(events(i,9) <= ifc_shifted(k),ifc_int(k)>1)) 
          %events_mod12(i,6) = 0;
          events_mod(i,11) = 1;
...
events_mod = sortrows(events_mod,11,'descend');
...
figure;
hold on;
h = histogram(events(:,9),binning,'DisplayStyle','stairs','DisplayName','C11 012');
%h_mod = histogram(events_spilloff(:,6),binning,'DisplayStyle','stairs','DisplayName','C11 012 masked 2');
h_mod = histogram(events_spilloff(:,9),binning,'DisplayName','C11 012 spill off');
...
%return;
%%
save('D:\PET_data_feb_2021\C11_012_spilloff.mat','events_spilloff','-v7.3');
save('D:\PET_data_feb_2021\C11_012_spillon.mat','events_spillon','-v7.3');
toc
%return;
