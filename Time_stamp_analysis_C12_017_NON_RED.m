...
z12=load('D:\PET_data_feb_2021\C12_017.mat');
...
for i=1:nevents
    ...
    if(events(i,9) >= ifc_shifted(length(ifc_shifted))-2*cycle-11000)
        for k = 0:spill_number 
            if and(events(i,9) >= (ifc_shifted(length(ifc_shifted))-spill+cycle*k), events(i,9) <= (ifc_shifted(length(ifc_shifted))+cycle*k))
    ...
    for k=2:length(ifc_hist)
       if and(events(i,9) >= ifc_shifted(k-1),and(events(i,9) <= ifc_shifted(k),ifc_int(k)>0)) 
          events_mod(i,11) = 1;
          n_spillon = n_spillon + 1;
          break;
       end
...
figure;
hold on;
h = histogram(events(:,9),binning,'DisplayStyle','stairs','DisplayName','C12 017');
h_mod = histogram(events_spilloff(:,9),binning,'DisplayName','C12 017 spill off');
...
%return;
%%
save('D:\PET_data_feb_2021\C12_017_spilloff.mat','events_spilloff','-v7.3');
save('D:\PET_data_feb_2021\C12_017_spillon.mat','events_spillon','-v7.3');
