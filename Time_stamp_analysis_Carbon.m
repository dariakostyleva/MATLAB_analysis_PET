...
% ifc_hist = load("hist_0255_C10_014_100msec_bins.txt");
% ifc_shifted = ifc_hist(:,1)*1000.-12250;
...
% ifc_hist = load("hist_0219_C10_001_100msec_bins.txt");
...
stairs(h_e,h_c,'DisplayName','PET data');
hold on;
stairs(ifc_shifted,ifc_int/1,'DisplayName','IFC data');
legend;
%return;
...
for i=1:nevents
    ...
    for k=2:length(ifc_hist)
       %k
       if and(events(i,6) >= ifc_shifted(k-1),and(events(i,6) <= ifc_shifted(k),ifc_int(k)>5)) 
          %events_mod12(i,6) = 0;
          events_mod(i,7) = 1;
...
figure;
hold on;
h = histogram(events(:,6),binning,'DisplayStyle','stairs','DisplayName','C11 007');
%h_mod = histogram(events_spilloff(:,6),binning,'DisplayStyle','stairs','DisplayName','C11 012 masked 2');
h_mod = histogram(events_spilloff(:,6),binning,'DisplayName','C11 007 spill off');
stairs(ifc_shifted,ifc_int/100,'DisplayName','IFC data');
hold off;
legend;

%%
save('D:\PET_data_feb_2021\C11_007_red_spilloff.mat','events_spilloff','-v7.3');
save('D:\PET_data_feb_2021\C11_007_red_spillon.mat','events_spillon','-v7.3');
toc
